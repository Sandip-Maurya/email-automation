"""Step implementations for the email automation pipeline.

These functions are used by both the legacy orchestrator and the LlamaIndex workflow.
Each step performs one stage of the pipeline (classify, extract, trigger, draft, etc.)
...
from src.models.outputs import AggregatedContext, DraftEmail, FinalEmail, ReviewResult
...
from src.agents.s3_scaffold import step_s3_ad1, step_s3_ad2, step_s3_ad3, step_s3_ad4
...
T = TypeVar("T")
logger = get_logger("email_automation.orchestrator_steps")
...
async def maybe_await(value: T | asyncio.Future[T] | Any) -> T:
...
async def step_classify(thread: EmailThread, tracer: Any) -> Any:
    ...
    a0_attrs = {
        ...
        **span_attributes_for_workflow_step(
            "CHAIN", input_summary={"thread_id": thread_id, "subject": original_subject}
        ),
    }
    with tracer.start_as_current_span("A0_classify", kind=SpanKind.INTERNAL, attributes=a0_attrs) as a0_span:
        ...
        set_span_input_output(
            a0_span, output_summary={"scenario": decision.scenario, "confidence": decision.confidence}
        )
    return decision


async def step_extract(thread: EmailThread, scenario: str, scenario_cfg: dict, tracer: Any) -> Any:
    ...
    extract_attrs = {
        ...
        **span_attributes_for_workflow_step(
            "TOOL", input_summary={"input_agent": input_agent_id, "scenario": scenario}
        ),
    }
...
async def step_trigger_fetch(
    ...
    trigger_attrs = {
        ...
        "workflow.scenario": scenario,
        **span_attributes_for_workflow_step(
            "TOOL", input_summary={"trigger": trigger_name, "scenario": scenario}
        ),
    }
...
async def step_draft(
    ...
    draft_attrs = {
        ...
        "workflow.scenario": scenario,
        **span_attributes_for_workflow_step(
            "TOOL", input_summary={"draft_agent": draft_agent_id, "scenario": scenario}
        ),
    }
    with tracer.start_as_current_span("draft", kind=SpanKind.INTERNAL, attributes=draft_attrs) as draft_span:
        ...
        set_span_input_output(
            draft_span,
            output_summary={"scenario": scenario, "trigger_source": draft.metadata.get("trigger_source", "")},
        )
    return draft


async def step_a11_aggregate(decision: Any, inputs: Any, tracer: Any) -> AggregatedContext:
    """Step: Input A11 aggregate -> AggregatedContext (Decision, NDC, Distributor, Year) for Decision A10."""
    a11_attrs = {
        "agent.name": "A11",
        **span_attributes_for_workflow_step("TOOL", input_summary={"decision": decision.scenario}),
    }
...
async def run_s3_scaffold(inputs: Any, tracer: Any) -> dict[str, Any]:
...
async def step_review(draft: Any, context_for_review: dict, scenario: str, tracer: Any) -> ReviewResult:
...
async def step_reply(
    ...
    reply_attrs = {
        ...
        **span_attributes_for_workflow_step(
            "TOOL", input_summary={"reply_to_message_id": reply_to_message_id}
        ),
    }
...
async def run_draft_or_send(
    ...
    """Handle DRAFT_ONLY (create draft + persist) or send reply. Returns updated raw_data."""
    log = logger.bind(thread_id=thread_id)
    if DRAFT_ONLY:
        log.debug("workflow.draft_only", reply_to_message_id=reply_to_message_id)
        async with _draft_creation_lock:
            if await asyncio.to_thread(email_outcome_has_recent_draft, thread_id):
                log.info(
                    "workflow.skip_draft_recent_exists",
                    thread_id=thread_id,
                ...
                log.info("workflow.draft_created", draft_message_id=draft_msg.id)
    else:
        log.debug("workflow.reply_attempt", reply_to_message_id=reply_to_message_id)
        sent_msg = await step_reply(provider, reply_to_message_id, final_email, user_id, tracer)
        raw_data["sent_message_id"] = sent_msg.id
        raw_data["sent_at"] = sent_msg.receivedDateTime
        log.info("workflow.reply_complete", sent_message_id=sent_msg.id)
    return raw_data
